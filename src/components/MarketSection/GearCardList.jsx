...
export default function GearCardList({ items, statColumns, renderBuyButton }) {
  if (items.length === 0) {
  ...
  return (
    <div className="sr-card-list">
      {items.map((item) => (
        <GearCatalogueCard key={item.id} item={item} statColumns={statColumns} renderBuyButton={renderBuyButton} />
      ))}
    </div>
  );
}
